	...
	return func(next wd.OnSessionQuit) wd.OnSessionQuit {
		return func(ctx *wd.Context, s *session.Session) error {
			...
			if shouldFinishTest {
				...
				rp := gorp.NewReportingClient(settings.Host, project, gorp.WithApiKeyAuth(ctx, settings.AuthToken))

				_, err = rp.FinishTest(ctx, itemID, &openapi.FinishTestItemRQ{
					LaunchUuid: launchID,
					Status:     new(string(gorp.Statuses.Passed)),
				})
				if err != nil {
...
func beforeSessionCreated(sr settingsRepo) func(next wd.OnBeforeSessionStart) wd.OnBeforeSessionStart {
	return func(next wd.OnBeforeSessionStart) wd.OnBeforeSessionStart {
		return func(ctx *wd.Context, prq *httputil.ProxyRequest, sessionRQ *wdproto.NewSessionRQ, sID string) error {
			...
			if launchID == "" {
				u := uuid.New()
				newLaunch, err := rp.StartLaunch(ctx, &openapi.StartLaunchRQ{
					Name:        "Browserkube",
					Description: new("Browser session: " + sID),
					Uuid:        u.String(),
					StartTime:   time.Now(),
					Attributes:  []openapi.ItemAttributesRQ{{Key: new("session"), Value: sID}},
					Mode:        new(string(gorp.LaunchModes.Default)),
				})
				if err != nil {
			...
			if itemID == "" {
				newTest, err := rp.StartTest(ctx, &openapi.StartTestItemRQ{
					Type:       string(gorp.TestItemTypes.Test),
					UniqueId:   new(uuid.New().String()),
					LaunchUuid: launchID,
					HasStats:   new(true),
					Retry:      new(false),
					TestCaseId: new(uuid.NewString()),
...
func afterCommandHandler(sr settingsRepo) func(next wd.OnAfterCommand) wd.OnAfterCommand {
	return func(next wd.OnAfterCommand) wd.OnAfterCommand {
		return func(ctx *wd.Context, rs *http.Response, sess *session.Session, command string) error {
			...
			if _, err = rp.SaveLog(ctx, &openapi.SaveLogRQ{
				ItemUuid:   new(itemID),
				LaunchUuid: launchID,
				Level:      new(gorp.LogLevelInfo),
				Time:       time.Now(),
				Message:    new(fmt.Sprintf("WebDriver command: %s %s", rs.Request.Method, command)),
			}); err != nil {
...
func findElementHandler(sr settingsRepo) func(next wd.OnAfterCommand) wd.OnAfterCommand {
	return func(next wd.OnAfterCommand) wd.OnAfterCommand {
		return func(ctx *wd.Context, rs *http.Response, sess *session.Session, command string) error {
			...
			_, err = gorp.NewReportingClient(settings.Host, project, gorp.WithApiKeyAuth(ctx, settings.AuthToken)).SaveLogMultipart(
				ctx,
				[]*openapi.SaveLogRQ{
					{
						LaunchUuid: launchID,
						ItemUuid:   new(itemID),
						Level:      new(gorp.LogLevelError),
						Time:       time.Now(),
						Message: new(fmt.Sprintf(
							"Element not found for item '%s', command: %s %s",
							itemID, rs.Request.Method, command,
						)),
						File: &openapi.File{Name: new(fileName)},
					},
				},
				[]gorp.Multipart{
